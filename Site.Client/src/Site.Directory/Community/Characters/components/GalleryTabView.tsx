...
const GalleryTabView: React.FC<GalleryTabViewProps> = ({ images }) => {
    ...
    const pckry = useRef<Packery | null>(null);

    useEffect(() => {
        if (gridRef.current && images.length > 0) {
            pckry.current = new Packery(gridRef.current, {
        ...
        <div ref={gridRef} className={styles.grid}>
            {images.map((image) => (
                <div
                    ...
                    style={{
                        width: `${(image.imageScale ?? 1) * 100}px`,
                        height: `${(image.imageScale ?? 1) * 100}px`,
                    }}
                >
                    <img
                        src={image.characterImageUrl}
                        alt={image.imageCaption || 'Character Image'}
