...
const GalleryTab = forwardRef<GalleryTabHandle, GalleryTabProps>(
    ({ characterId, images, onGalleryUpdate, onImageUpload }, ref) => {
        ...
        useEffect(() => {
            if (gridRef.current && !pckry.current) {
                ...
                pckry.current.on('dragItemPositioned', () => {
                    ...
                    if (itemElems) {
                        const newImageIds = itemElems.map((elem: HTMLElement) =>
                            parseInt(elem.getAttribute('data-id') || '0', 10)
                        );
        ...
        const handleCaptionChange = (id: number, caption: string) => {
            const newImages = currentImages.map((img) =>
                img.characterImageId === id ? { ...img, imageCaption: caption } : img
            );
        ...
        const handleImageScaleChange = (id: number, scale: number) => {
            const newImages = currentImages.map((img) =>
                img.characterImageId === id ? { ...img, imageScale: scale } : img
            );
            setCurrentImages(newImages);
        ...
        const handleUpdateGallery = async (e: React.FormEvent) => {
            ...
            try {
                await axios.put(`/api/characters/${characterId}/gallery/update`, {
                    images: updates,
                    imagesToDelete: pendingDeletions,
                });
        ...
        const handleUploadNewImages = async (filesToUpload: FileList | null) => {
            ...
            try {
                const response = await axios.post<CharacterImage[]>(
                    `/api/characters/${characterId}/gallery/upload`,
                    formData
                );
                    ...
                    <div className="col-lg-8 d-flex flex-column">
                        <div className="card h-100">
                            <div className="card-body d-flex flex-column">
                                <div
                                    ref={gridRef}
                                    className={`grid w-100 h-100 ${styles.gridContainer}`}
                                ...
                                >
                                    {currentImages.map((image) => (
                                        <div
                                            ...
                                            style={getGridItemStyle(image)}
                                            data-id={image.characterImageId}
                                        >
                                            <img
